...
test.describe('/search-prisoner/add-court-appearance unauthorised', () => {
  test('should show unauthorised error', async ({ page }) => {
    await testNotAuthorisedPage(page, '/search-prisoner/add-court-appearance')
  })
})
...
test.describe('/search-prisoner/add-court-appearance', () => {
  ...
  test('should try all cases', async ({ page }) => {
    await page.goto('/search-prisoner/add-court-appearance')
    ...
    await expect(testPage.link('Create a court appearance for Name One')).toHaveAttribute(
      'href',
      /\/add-court-appearance\/start\/A1234EA/,
    )

    await testPage.verifyTableRow(2, ['Another Two', 'A2345EA', 'Loc-2'])
    await expect(testPage.link('Create a court appearance for Another Two')).toHaveAttribute(
      'href',
      /\/add-court-appearance\/start\/A2345EA/,
    )
  })
})
