...
test.describe('/search-prisoner/view-court-appearance-history', () => {
  ...
  test('should try all cases', async ({ page }) => {
    await page.goto('/search-prisoner/view-court-appearance-history')

    // verify page content
    const testPage = await new AddCourtAppearanceSearchPrisonerPage(page).verifyContent()
    ...
    await expect(testPage.link('View court appearance history for Name One')).toHaveAttribute(
      'href',
      /\/view-court-appearance-history\/A1234EA/,
    )

    await testPage.verifyTableRow(2, ['Another Two', 'A2345EA', 'Loc-2'])
    await expect(testPage.link('View court appearance history for Another Two')).toHaveAttribute(
      'href',
      /\/view-court-appearance-history\/A2345EA/,
    )
  })
})
