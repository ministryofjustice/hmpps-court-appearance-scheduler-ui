...
import { CourtAppearanceDateTimePage } from './test.page'
...
test.describe('/add-court-appearance/date-and-time unauthorised', () => {
  test('should show unauthorised error', async ({ page }) => {
    await testNotAuthorisedPage(page, '/add-court-appearance/date-and-time')
  })
})

test.describe('/add-court-appearance/date-and-time', () => {
  ...
  const startJourney = async (page: Page, journeyId: string) => {
    ...
    await page.goto(`/${journeyId}/add-court-appearance/date-and-time`)
  }
  ...
  test('should try all cases', async ({ page }) => {
    ...
    // verify page content
    const testPage = await new CourtAppearanceDateTimePage(page).verifyContent()
    ...
    expect(page.url()).toMatch(/\/add-court-appearance\/details/)
